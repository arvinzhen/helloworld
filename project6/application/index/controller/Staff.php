...
class Staff extends Common{
	...
	public function alllist($pid){
		...
		$res2 = Db::table('staff')->field('classid,title,intro,releasetime')->where('classid','in',$classids)->paginate(5,false)->each(function($item,$key){
	...
	public function newsdetail($classid,$pid){
		...
		$page = updown($res1,$classid,"@index/staff/show/");
		$res = Db::table('staff')->field("id,title,content,releasetime,viewpage")->where('classid','=',$classid)->find();
	...
	public function addv($id){
		$res = Db::table('staff')->field('viewpage')->where('id','=',$id)->find();
		if(empty($res)){
			die();
		}
		$views = $res['viewpage'] + 1;
		Db::table('staff')->where('id','=',$id)->data(['viewpage'=>$views])->update();
		die();
	}
}
