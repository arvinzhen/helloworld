...
class Staff extends Controller
{
    ...
    protected $alltype = 4;//classes里面的分类
    ...
    public function index()
    {
    	...
    	$res2 = Db::table('staff')->field('classid,title,releasetime')->where($condition2)->order('id','asc')->paginate(8,false,['query'=>$query2])->each(function($item,$key){
    ...
    public function save(Request $request)
    {
    	...
    	$tag1 = Db::table('staff')->data($data)->insert();
    ...
    public function read($classid = 0)
    {
        ...
        $res = Db::table('staff')->where('classid','=',$classid)->find();
    ...
    public function edit($classid = 0)
    {
        ...
        $res2 = Db::table('staff')->field('id,title,intro,content')->where('classid','=',$classid)->find();
    ...
    public function update(Request $request, $classid)
    {
    	...
    	if(!$title){echo ajson('数据异常,未获得标题');die();}
    	//原始数据
    	$olddata1 = Db::table('staff')->field('title,intro,content')->where('classid','=',$classid)->find();
    	...
    	$tag1 = true;$tag2 = true;
    	if($data){
    		$tag1 = Db::table('staff')->where('classid','=',$classid)->update($data);
    	}
    	if($data2){
    ...
    public function delete($classid = 0)
    {
        ...
        $res = Db::table('staff')->field('content')->where('classid','=',$classid)->find();
        if($res && $res['content']){
        ...
        $tag1 = Db::table('staff')->where('classid','=',$classid)->delete();
