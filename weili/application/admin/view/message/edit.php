	...
	<div class="layui-fluid">
		<form class="layui-form">
			...
			<div class="layui-form-item">
				...
				<div class="layui-input-inline">
					<input type="text" name="phone" id="phone" autocomplete="off" required="" lay-verify="phone" class="layui-input" value="{$data.phone}">
				</div>
			</div>
			<div class="layui-form-item">
				<label for="l-express" class="layui-form-label">您的留言:</label>
				<div class="layui-input-inline">
					<textarea rows="5" name="content" class="shuruqu">{$data.content}</textarea>
				<br class="clearfix" />
				</div>
			</div>
			<div class="layui-form-item">
				...
				<div class="layui-input-inline">
					<button id="add"  class="layui-btn" lay-filter="add" lay-submit="">更新</button>
				</div>
			</div>
		</form>
	</div>
	...
	<script>
		layui.use(['form','layer'],function(){
			...
			form.on('submit(add)',function(data){
				...
				$.ajax({
					url:'{:url('@admin/message/update/'.$data.classid)}'
					,method:'POST'
					,data:data.field
					,dataType:'json'
