...
class Company extends Controller
{
    ...
    protected $alltype = 3;//classes里面的分类
    ...
    public function index()
    {
    	...
    	$res2 = Db::table('company')->field('classid,title,releasetime')->where('classid','in',$classid)->order('id','asc')->paginate(8)->each(function($item,$key){
    ...
    public function save(Request $request)
    {
    	...
    	$data = ['classid'=>$classid,'title'=>$title,'content'=>$content,'releasetime'=>$releasetime];
    	$data2 = ['pid'=>$pid,'classid'=>$classid,'title'=>$title,'type'=>$this->alltype];
    	$tag1 = Db::table('company')->data($data)->insert();
    ...
    public function read($classid = 0)
    {
        ...
        $res = Db::table('company')->where('classid','=',$classid)->find();
    ...
    public function edit($classid = 0)
    {
        ...
        $res2 = Db::table('company')->field('id,title,content')->where('classid','=',$classid)->find();
    ...
    public function update(Request $request, $classid)
    {
    	...
    	if(!$title){echo ajson('数据异常,未获得标题');die();}
    	//原始数据
    	$olddata1 = Db::table('company')->field('title,content')->where('classid','=',$classid)->find();
    	...
    	$tag1 = true;$tag2 = true;
    	if($data){
    		$tag1 = Db::table('company')->where('classid','=',$classid)->update($data);
    	}
    	if($data2){
    ...
    public function delete($classid = 0)
    {
        ...
        $res = Db::table('company')->field('content')->where('classid','=',$classid)->find();
        if($res && $res['content']){
        ...
        $tag1 = Db::table('company')->where('classid','=',$classid)->delete();
