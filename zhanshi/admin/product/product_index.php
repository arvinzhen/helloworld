<?php 
    ...
    $condition = "";
    if(!$search){
        $sql = "select id,name,classid,visit,releasetime from product order by id asc".$limit;
    }else{
        $condition = substr($search,3);
        $sql = "select id,name,classid,visit,releasetime from product where".$condition." order by id asc".$limit;
    }
    // ohno($search);
    ...
    if($res){
        foreach ($res as $value) {
            ...
            $str .= "<td class='td-manage'><a title='编辑'  onclick=\"xadmin.open('编辑','product_edit.php?id=$id&classid=$classid',800,800)\" href='javascript:;'><i class='layui-icon'>&#xe642;</i>编辑</a></td>";
            $str .= "<td class='td-manage'><a onclick=\"xadmin.open('详情','product_detail.php?id=$id',800,800)\" title='详情' href='javascript:;'><i class='layui-icon'>&#xe631;</i>详情</a></td>";
            ...
            <div class="layui-card">
                <div class="layui-card-body ">
                    <form class="layui-form layui-col-space4" action="" method='POST' autocomplete="off">
                        ...
                        <div class="layui-inline layui-show-xs-block">
                            <input type="text" name="name"  placeholder="请输入案例A标题" autocomplete="off" class="layui-input">
                        </div>
                        <div class="layui-inline layui-show-xs-block">
                ...
                <div class="layui-card-header">
                    ...
                    <button class="layui-btn" onclick="xadmin.open('添加用户','/admin/product/product_add.php',800,600)"><i class="layui-icon"></i>添加</button>
                </div>
                ...
                <?php if($res){bottomNav("product","product_index.php",$curPage, $condition);} ?>
            </div>
        </div>
    </div>
</div> 
