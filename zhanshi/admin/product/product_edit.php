<?php 
	include("../../zhanshi.php");
	checkLogin();
	if(!$_GET){echo "<script>hostory.back();</script>";die();}
	$id = $_GET["id"];
	$sql = "select name,content,image from product where id=$id";
...
<head>
 		...
 		<div class="layui-row">
 			<form class="layui-form layui-col-md7">
 				<div class="layui-from-item">
	 					<label class="layui-form-label">A案例标题</label>
	 					<div class="layui-input-inline">
		 					<input type="text" name="name" class="layui-input" required autocomplete="off" value="<?php echo $name; ?>">
	 					</div>
 				...
 				<div class="layui-form-item layui-form-text">
 					...
 					<div class="layui-input-block">
 						<textarea name="content" cols="50" rows="10"><?php echo $content; ?></textarea><span class="x-red">内容</span>
 					</div>
 				</div>
 				<div class="form-item">
	 		...
	 		<div class="layui-col-md5" style="border:1px solid red">
	 				...
	 				<div><img src="/images/<?php echo $image;?>" alt=""></div>
	 		</div>
 		</div>
 	</div>
 </body>
 <script>
 	var picList;
 	<?php echo "var id=$id,classid=$classid;"; ?>
 	$("#image").change(function(){
 		...
 		$.ajax({
 			url:'/admin/product/product_update_save.php'
 			,method:'POST'
 			,data:test
 			,dataType:'json'
 			,async:false
	 ...
	 layui.use('form', function(){
	 	...
	 	form.on('submit(tijiao)',function(data){
	 		allData = {'data':data.field,"id":id,"classid":classid};
		 	if(picList){
		 		allData['img'] = picList[0];
		 	}else{
		 		allData['img'] = '';
		 	}
	 		...
	 		$.ajax({
	 			...
	 			,success:function(res){
	 				if(res.state == 1){
	 					...
	 					layer.msg(res.state,{icon:3,time:1800},function(){
	 						// xadmin.close();
	 						xadmin.father_reload();
	 					})
	 				}
 	...
 	// 		elem:'#image'
 	// 		,url:'/admin/awoks/product_add_save.php'
 	// 		,done:function(){
 	// 			//
 	// 		}
 	// 		,error:function(){
