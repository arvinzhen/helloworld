<?php 
	include("../../zhanshi.php");
	checkLogin();
	$sql = "select name,sontype from classes where type=2 and classid=0";
	$res = sqlquery("zhanshi",$sql);
	$str = "";
	if($res){//少判断了sontype为1次的情况
 		...
 		<div class="layui-row">
 			<form class="layui-form layui-col-md7">
 				...
 				<div class="layui-input-item">
 					<label for="sontype" class="layui-from-label">分类</label>
 					<div class="layui-input-inline">
	 					<select name="sontype" id="sontype" class="layui-select">
	 						<option value="">如果没有的话,请去总表加</option>
	 						<?php echo $str; ?>
	 					</select>
 					</div>
 				</div>
	 ...
	 layui.use('form', function(){
	 	...
	 	form.on('submit(tijiao)',function(data){
	 		if(!data.field.sontype){
	 			layer.msg('当前未选择类别',{icon:0,time:1200});return false;
	 		}
	 		if(!data.field.name){
	 			layer.msg('标题不能为空哦',{icon:0,time:1200});return false;
	 		}
		 	if(picList){
