<?php 
	...
	//加载当前页的主体内容
	$strzhuti = "";$jsstr = "";$fyclass = "";$xsName = "案例中心";
	$curPage = isset($_GET['page'])?$_GET['page']-1:0;
	if(!isset($_GET['classid'])){//显示概括
		$strfenye = "";
		$search = "";$limit = "";$sql = "select classid from classes where type=2 and classid>0 ";//iniliz
		...
		if($sontype){
			...
			$sql3 = "select name from classes where type=2 and classid=0 and sontype='$sontype'";
		...
		if($res){//有内容
			...
			if(count($res) > 4){//fenye
				...
				if($curPage > 0){
					$i = $curPage;
					$strfenye .= "<a href='/admin/aworks/index.php?".$fyclass."&page=$i'>上一页</a>";
				}
				for($i = 1; $i <= $nums; $i++){
					if($i - 1 == $curPage){
						$strfenye .= "<a href='/admin/aworks/index.php?".$fyclass."&page=$i' disabled>$i</a>";
					}else{
						$strfenye .= "<a href='/admin/aworks/index.php?".$fyclass."&page=$i'>$i</a>";
					}
				}
				if($curPage < $nums-1){
					$i = $curPage + 2;
					$strfenye .= "<a href='/admin/aworks/index.php?".$fyclass."&page=$i'>下一页</a>";
				}
				$strfenye .= "</div>";
				//处理第一页显示
				$sql .= $limit;
			...
			$sql = "select name,image from aworks where classid in($classids) order by id asc";
			$res = sqlquery("zhanshi",$sql);
			//正式写入容器
			if($res){
				for ($i = 0; $i < count($res); $i++) {
					...
					$strzhuti .= "<p class='layui-inline'><a href='/admin/aworks/index.php?classid=$classid'>$name</a></p>";
					$strzhuti .= "</div>";
				}
				$strzhuti .= $strfenye;
			}
		...
		if($classid){
			$sql = "select * from aworks where classid=$classid";
	...
	<div class="layui-main zhuti">
		<div class="zhuti-left layui-inline">
			<h3 class="layui-block">
				W
				<p class="layui-inline">orks<br/><span>案例中心</span></p>
			</h3>
			<div>
				<?php echo $arr2[1]; ?>
			</div>
...
<script>
	...
	if(tag){
		setTimeout(function(){
			$.ajax({
				url:'/admin/aworks/work_add_save.php',
