<?php 
	...
	$sql = "select id,name from about order by id";
	$res2 = sqlquery("zhanshi",$sql);
	if($res2){
		$ulStr = "";
		foreach ($res2 as $value) {//这里没有判断空的情况
			$id = $value['id'];
			$name = $value['name'];
			$ulStr .= "<li><a href='/admin/about/index.php?id=$id'>$name</a></li>";
		}
		if($lid){//确定已有内容情况下,为了不必要的开销
			$sql = "select name,content from about where id=$lid";
			$xsStr = sqlquery("zhanshi",$sql,false)['content'];
			$xsName = sqlquery("zhanshi",$sql,false)['name'];
		}else{
			$sql = "select content from about order by id asc limit 0,1";
			$xsStr = sqlquery("zhanshi",$sql,false)['content'];
			$xsName = "公司简介";
		}
	}else{
	...
	<div class="layui-main zhuti">
		<div class="zhuti-left layui-inline">
			<h3 class="layui-block">
				A
				<p class="layui-inline">bouts<br/><span>公司简介</span></p>
			</h3>
			<div>
				<?php echo $ulStr; ?>
			</div>
