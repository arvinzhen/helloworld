<?php 
	...
	if(!isset($_POST['visit'])){
		...
		if($_POST){//处理表单
			...
			if(!$_POST["data"]["sontype"]){echo json_encode(['state' => '类别没选哦!']);die();}
			...
			$id = $res["classid"]+1;
			$releasetime = time();
			$sql = "insert into bworks set name='$name',content='$content',image='$img',classid='$id',releasetime='$releasetime'";
			$tag = sqlexec("zhanshi",$sql);
			$sql = "insert into classes set name='$name',type='4',classid='$id',sontype='$sontype'";
		...
		$sql = "update bworks set visit='$vis' where classid=$classid";
		sqlexec("zhanshi",$sql);die();
	}
