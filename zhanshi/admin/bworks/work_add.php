<?php 
	include("../../zhanshi.php");
	checkLogin();
	$sql = "select sontype,name from classes where type=4 and classid=0";
	...
	if($res){
		...
		$str = "<option value=''>Fuck You!</option>";
	}
 ?>
<!DOCTYPE html>
 				...
 				<div class="layui-form-item">
 					...
 					<div class="layui-input-inline">
 						<select name="sontype" id="sontype" class="layui-select">
 							<option value="">必选哦,如果没有请去主页加类别</option>}
 							<?php echo $str; ?>
 						</select>
 					</div>
 				</div>
	 ...
	 layui.use('form', function(){
	 	...
	 	form.on('submit(tijiao)',function(data){
	 		...
	 		if(!data.field.sontype){
	 			layer.msg('类别没选哦!',{icon:0,time:1200});return false;
	 		}
		 	if(picList){
