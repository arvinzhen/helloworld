...
use app\yteng\model\News;
use sort\Sortclass;
use think\Db;
class Newsdo extends Par{
	...
	public function edit(Sortclass $sort ,$id=0){
		if($id){
			//编辑时， 需要返回当前类别的所有信息
			$data["row"] = News::where('id',$id)->find();
	...
	public function del(){
		...
		$result = News::where("id",'in',$ids)->field('pic')->select();
		foreach ($result as $v) {
			if($v["pic"]){
	...
	public function getList(){
        ...
        //搜索条件的建立 end
		$data["list"] = News::where($map)->order('id', 'desc')->paginate(10,false,[
		    'query'     => $query,
		    'path'      => url("yteng/Newsdo/index")
		]);
